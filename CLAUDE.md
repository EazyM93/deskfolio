...
This file is DeskFolio's permanent engineering handbook. It must be committed to Git and kept concise. Do not store active tasks, session summaries, temporary plans, backlog state or progress logs here. Those belong in `.ai/`, which is local-only and ignored by Git.
...
Claude must update local AI memory after meaningful work:
